...
//-----------------------------------------------------------------------------
// 
//  ObjLoaderEx class declaration 
//
//-----------------------------------------------------------------------------
class ObjLoaderEx
{
public:
  ObjLoaderEx( const std::string& filename,           // Model filename
                      optix::Context context,               // Context for RT object creation
                      optix::GeometryGroup geometrygroup ); // Empty geom group to hold model
  ObjLoaderEx( const std::string& filename,
  ...
  ~ObjLoaderEx() {} // makes sure CRT objects are destroyed on the correct heap

  void setIntersectProgram( optix::Program program );
  void load();
  void load( const optix::Matrix4x4& transform );

  optix::Aabb getSceneBBox()const { return _aabb; }
  optix::Buffer getLightBuffer()const { return _light_buffer; }

...
void PpmObjLoader::load() 
{
  ...
  if ( !model ) {
    std::stringstream ss;
    ss << "PpmObjLoader::loadImpl - glmReadOBJ( '" << _filename << "' ) failed" << std::endl;
...
void PpmObjLoader::createMaterial() 
{
  ...
  _material->setClosestHitProgram( 0u, closest_hit1 );
  _material->setClosestHitProgram( 1u, closest_hit2 );
  _material->setAnyHitProgram( 2u, any_hit );
}

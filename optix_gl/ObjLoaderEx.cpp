...
//------------------------------------------------------------------------------
// 
//  ObjLoaderEx class definition 
//
//------------------------------------------------------------------------------

ObjLoaderEx::ObjLoaderEx( const std::string& filename,
                      Context context,
...
void ObjLoaderEx::load()
{
  load( optix::Matrix4x4::identity() );
}
void ObjLoaderEx::setIntersectProgram( Program intersect_program )
{
  _intersect_program = intersect_program;
}

void ObjLoaderEx::load( const optix::Matrix4x4& transform )
{
  // parse the OBJ file
  GLMmodel* model = glmReadOBJ( _filename.c_str() );
  if ( !model ) {
    std::stringstream ss;
    ss << "ObjLoaderEx::loadImpl - glmReadOBJ( '" << _filename << "' ) failed" << std::endl;
  ...
  if( !_intersect_program.get() ) {
    ...
    _intersect_program = m_context->createProgramFromPTXFile( path, "mesh_intersect" );
  }
...
void ObjLoaderEx::createMaterial() 
{
  ...
  _material           = m_context->createMaterial();
  _material->setClosestHitProgram( 0u, closest_hit );
  _material->setAnyHitProgram( 1u, any_hit );
}


void ObjLoaderEx::loadVertexData( GLMmodel* model, const optix::Matrix4x4& transform )
{
  ...
  _vbuffer = m_context->createBuffer( RT_BUFFER_INPUT, RT_FORMAT_FLOAT3, num_vertices );
  float3* vbuffer_data = static_cast<float3*>( _vbuffer->map() );

  // Create normal buffer
  _nbuffer = m_context->createBuffer( RT_BUFFER_INPUT, RT_FORMAT_FLOAT3, num_normals );
  float3* nbuffer_data = static_cast<float3*>( _nbuffer->map() );

  // Create texcoord buffer
  _tbuffer = m_context->createBuffer( RT_BUFFER_INPUT, RT_FORMAT_FLOAT2, num_texcoords );
...
/////////////////////////////////////////////////////////////////////////
  ...
  for ( unsigned int i = 0; i < num_vertices; ++i )
  {
		...
		vbuffer_data[i] = make_float3(x,y,z);
  }
  
  for ( unsigned int i = 0; i < num_vertices; ++i )
  {
		float& x = vbuffer_data[i].x;
		float& y = vbuffer_data[i].y;
		float& z = vbuffer_data[i].z;
...
void ObjLoaderEx::createGeometryInstances( GLMmodel* model,
                                         ...
                                         Program mesh_bbox )
{
  ...
  for ( GLMgroup* obj_group = model->groups;
    ...
    GeometryInstance instance = m_context->createGeometryInstance( mesh, &_material, &_material+1 );
...
bool ObjLoaderEx::isMyFile( const std::string& filename )
{
  return getExtension( filename ) == "obj";
}


void ObjLoaderEx::loadMaterialParams( GeometryInstance gi, unsigned int index )
{
  ...
  std::cerr << "WARNING -- ObjLoaderEx::loadMaterialParams given index out of range: "
            << index << std::endl;
}


void ObjLoaderEx::createMaterialParams( GLMmodel* model )
{
...
void ObjLoaderEx::createLightBuffer( GLMmodel* model )
{
  // create a buffer for the next-event estimation
  _light_buffer = m_context->createBuffer( RT_BUFFER_INPUT );
